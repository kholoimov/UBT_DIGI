...
constexpr std::array<int, 15> kTimingThresholdPe = {
...
double gammaComponent(double x, double area, double x0, double k,
  ...
  if (z <= 0.0 || area < 0.0 || k <= 0.0 || theta <= 0.0) return 0.0;
...
double doubleShiftedGamma(double* x, double* par) {
  const double peak = gammaComponent(x[0], par[0], par[1], par[2], par[3]);

  const double tail = gammaComponent(x[0], par[4], par[1], par[5], par[6]);

  return peak + tail;
}

void plot_event_observables(const char* inputFile = "scintillator_digi.root",
  ...
  for (std::size_t i = 0; i < kStoredArrivalPe.size(); ++i) {
    const TString branchName = TString::Format(
        "photoelectron_arrival_%d_from_muon_ns", kStoredArrivalPe[i]);
  ...
  for (int index : kOverlayIndices) {
    overlayHistograms.push_back(std::make_unique<TH1D>(
        ...
        TString::Format("Arrival Time Overlay;Arrival time from muon "
                        "[ns];Normalized entries"),
        120, 0.0, 6.0));
  }
      ...
      for (std::size_t j = 0; j < kOverlayIndices.size(); ++j) {
        ...
        if (overlayIt != kStoredArrivalPe.end()) {
          const std::size_t index = static_cast<std::size_t>(
              std::distance(kStoredArrivalPe.begin(), overlayIt));
  ...
  {
    TCanvas canvas("produced_photoelectrons", "produced_photoelectrons", 1100,
                   800);
    ...
    double fitMin =
        pmt_charge_pc_histogram->GetXaxis()->GetBinLowEdge(firstBin);
    ...
    */
    ...
    fitFunction->SetParNames("Peak area", "Threshold x_{0}", "Peak k",
                             "Peak #theta", "Tail area", "Tail k",
                             "Tail #theta");

    const double totalCounts = pmt_charge_pc_histogram->Integral(
        pmt_charge_pc_histogram->FindBin(fitMin),
    ...
    */
    fitFunction->SetParameters(0.65 * totalCounts,  // peak area
                               135.0,               // common x0
                               5.0,                 // peak k
    ...
    */
    TFitResultPtr result = pmt_charge_pc_histogram->Fit(fitFunction, "RLS0");

    pmt_charge_pc_histogram->Draw("HIST");
    pmt_charge_pc_histogram->SetStats(1);
    fitFunction->Draw("SAME");

    const double mostProbableValue = fitFunction->GetMaximumX(fitMin, fitMax);

    const double fittedMaximum = fitFunction->Eval(mostProbableValue);

    const double lineTop =
        TMath::Max(pmt_charge_pc_histogram->GetMaximum(), fittedMaximum) * 1.05;

    TLine* modeLine =
        new TLine(mostProbableValue, 0.0, mostProbableValue, lineTop);
     ...
     */
    TLatex* modeLabel =
        new TLatex(mostProbableValue + 2.0 * binWidth, 0.90 * lineTop,
  ...
  {
    TCanvas canvas("c_arrived_vs_photons",
                   "Arrived Photons vs Produced Photons", 1100, 800);
  ...
  {
    TCanvas canvas("c_pe_vs_photons", "Photoelectrons vs Produced Photons",
                   1100, 800);
  ...
  for (int thresholdPe : kTimingThresholdPe) {
    const auto thresholdIt = std::find(kStoredArrivalPe.begin(),
                                       kStoredArrivalPe.end(), thresholdPe);
    if (thresholdIt != kStoredArrivalPe.end()) {
      ...
      const double varianceNs = std::max(
          0.0, (arrivalTimeSqSumsNs[index] / arrivalTimeCounts[index]) -
                   meanNs * meanNs);
  ...
  if (pmtPhotonBirths != nullptr) {
    ...
    std::cerr
        << "No 'pmt_photon_births' tree found; birth-position map was not "
           "produced."
        << std::endl;
  }
  ...
  if (scintBirthTimeHistogram->GetEntries() > 0.0 ||
    ...
    const double maxCounts =
        std::max(scintBirthTimeHistogram->GetMaximum(),
    ...
    if (scintBirthTimeHistogram->GetEntries() > 0.0) {
      ...
      scintBirthFit.SetParameters(scintBirthTimeHistogram->Integral(), 0.02,
                                  2.5, std::max(0.05, peakX / 3.0));
...

  {
    ...
    for (std::size_t i = 0; i < overlayHistograms.size(); ++i) {
      ...
      hist->SetLineWidth(3);
      hist->SetTitle(
          "Arrival-Time Overlays for Selected Photoelectron Indices;Arrival "
          "time from muon [ns];Normalized entries");
  ...
  if (deltaArrivalTimePe1MinusPe100Histogram->GetEntries() > 0.0 ||
    ...
    const double maximum =
        std::max(deltaArrivalTimePe1MinusPe100Histogram->GetMaximum(),
    ...
    legend.AddEntry(deltaArrivalTimePe1MinusPe100Histogram.get(),
                    "t_{100} - t_{1}", "l");
    legend.AddEntry(arrivalTimePe100Histogram.get(), "t_{100}", "l");
    legend.Draw();

    SaveCanvas(canvas, outputDir, "arrival_time_pe1_minus_pe100_and_pe100");
  }

  {
    ...
    for (std::size_t i = 0; i < kStoredArrivalPe.size(); ++i) {
      if (arrivalTimeCounts[i] > 0) {
        ...
        meanArrivalTimesNs.push_back(arrivalTimeSumsNs[i] /
                                     arrivalTimeCounts[i]);
      }
    }
    ...
    if (!arrivalIndices.empty()) {
      ...
      TGraph graph(static_cast<int>(arrivalIndices.size()),
                   arrivalIndices.data(), meanArrivalTimesNs.data());
      graph.SetTitle(
          "Mean Arrival Time vs Photoelectron Index;Photoelectron index;Mean "
          "arrival time from muon [ns]");
      graph.SetMarkerStyle(20);
...

  {
    ...
    for (std::size_t i = 1; i < kStoredArrivalPe.size(); ++i) {
      ...
      const double varianceNs = std::max(
          0.0, deltaArrivalTimeSqSumsNs[i] / deltaArrivalTimeCounts[i] -
                   meanNs * meanNs);
    ...
    if (!deltaArrivalIndices.empty()) {
      ...
      graph.SetTitle(
          "Mean Arrival-Time Difference vs Threshold;Photoelectron "
          "threshold;Mean [t_{N} - t_{1}] from muon [ns]");
      graph.SetMarkerStyle(20);
      graph.SetMarkerSize(0.9);
      graph.SetMarkerColor(kBlue + 2);
      graph.SetLineColor(kBlue + 2);
      ...
      TCanvas sigmaCanvas("c_sigma_delta_arrival_time_vs_pe_index",
                          "Sigma of Arrival-Time Difference vs Threshold", 1000,
                          700);
      ...
      sigmaGraph.SetTitle(
          ...
          "threshold;Sigma of [t_{N} - t_{1}] [ns]");
      ...
      SaveCanvas(sigmaCanvas, outputDir,
                 "sigma_delta_arrival_time_vs_threshold");
    }
  }
}
