...
void EventAction::BeginOfEventAction(const G4Event*) {
  auto& eventData = EventData::Instance();
  eventData.Reset();
  eventData.SetEnableScintillatorPhotonStudies(
      fRunAction->GetEnableScintillatorPhotonStudies());
}
...
void EventAction::EndOfEventAction(const G4Event* event) {
  ...
  if (digi != nullptr) {
    ...
    G4cout << "[Event " << event->GetEventID()
           << "] Primary=" << digi->GetPrimaryParticle()
           << ", Ekin=" << digi->GetPrimaryKineticEnergy() / CLHEP::MeV
           << " MeV, p=" << digi->GetPrimaryMomentum() / CLHEP::MeV
           << " MeV/c, MuRange="
           << digi->GetPrimaryMuonTrackLength() / CLHEP::mm
           << " mm, Edep=" << digi->GetEnergyDeposit() / CLHEP::MeV
           << " MeV, Nscint=" << digi->GetScintillationPhotons()
           << ", PMTphot=" << digi->GetPmtIncidentPhotons()
           << ", PE=" << digi->GetDetectedPhotoelectrons()
           << ", ADC=" << digi->GetAdcCounts()
           << ", Q=" << digi->GetPmtCharge() / kPicocoulomb << " pC, t0="
           ...
           << (digi->GetThreshold80TimeFromPrimary() >= 0.0
                   ...
                   : -1.0)
           << " ns, Trigger=" << digi->GetTriggered() << G4endl;
  }
}
