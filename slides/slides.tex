...
\begin{frame}{Scintillation Timing Model Validation}
  \begin{columns}[T]
    \begin{column}{0.6\textwidth}
      \centering
      \includegraphics[height=3.8cm]{../analysis/scintillation_real_vs_generated.pdf}\\[2pt]
      \footnotesize Normalized measured spectrum compared with times
      generated from the fitted two-component model.
    \end{column}
    \begin{column}{0.4\textwidth}
      \centering
      \includegraphics[height=2.3cm]{../analysis/scintillation_real_data.pdf}\\[2pt]
...
\begin{frame}{Simulated Integrated PMT-Charge Distributions}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.0cm]{../analysis/plots/photoelectrons.pdf}\\[2pt]
      \footnotesize $40\times40~\mathrm{mm^2}$ tile
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.0cm]{../analysis/plots_20mm/photoelectrons.pdf}\\[2pt]
...
\begin{frame}{Spatial Uniformity of the ADC Response}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.0cm]{../analysis/plots/adc/mean_adc_vs_position.png}\\[2pt]
      \footnotesize Raw mean ADC vs.\ position
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.0cm]{../analysis/plots/adc/mean_adc_per_mev_vs_position.png}\\[2pt]
...
\begin{frame}{Individual Per-Particle Mean-ADC-vs-Energy Curves}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.0cm]{../analysis/plots/energy_scan/adc_vs_energy_muon.pdf}\\[2pt]
      \footnotesize \muminus
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.0cm]{../analysis/plots/energy_scan/adc_vs_energy_electron.pdf}\\[2pt]
      \footnotesize \eminus
    \end{column}
  \end{columns}
  \vspace{4pt}
  \footnotesize
  Muon response is essentially flat ($\appprox$ 170 ADC counts). The
  electron scan is peaking $\approx 190$ ADC
  at 4.3\,MeV, settling onto the 172--174-count plateau above
  13\,MeV.
\end{frame}
...
\begin{frame}{Photon Response, Split by Interaction}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.0cm]{../analysis/plots/energy_scan/adc_vs_energy_photon_all.pdf}\\[2pt]
      \footnotesize All \gam\ events, interacted or not
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.0cm]{../analysis/plots/energy_scan/adc_vs_energy_photon_interacted.pdf}\\[2pt]
...
\begin{frame}{Two Levels of the Final UBT Mapping}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.3cm]{../geometry/ubt_geometry_simulation.png}\\[2pt]
      \footnotesize Coarse transport map: 225 regions of $40\times40~\mathrm{cm^2}$
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=4.3cm]{../geometry/ubt_geometry_digitization.png}\\[2pt]
...
\begin{frame}{Expected Particle Rate per Tile}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=3.9cm]{figs/ubt_digitized_hits.png}\\[2pt]
      \footnotesize General spatial rate map, all species (weighted, log $z$)
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=3.9cm]{figs/ubt_digitized_hits_hits_per_tile.png}\\[2pt]
      \footnotesize Per-tile expected hit-rate distribution (log $y$)
    \end{column}
  \end{columns}
  \vspace{4pt}
  \footnotesize
...
\begin{frame}{Expected Rate per Particle Type}
  \begin{columns}[T]
    \begin{column}{0.333\textwidth}
      \centering
      \includegraphics[height=3.6cm]{figs/ubt_digitized_hits_muon.png}\\[2pt]
      \footnotesize \muminus\ only
    \end{column}
    \begin{column}{0.333\textwidth}
      \centering
      \includegraphics[height=3.6cm]{figs/ubt_digitized_hits_electron.png}\\[2pt]
      \footnotesize \eminus/$e^+$ only
    \end{column}
    \begin{column}{0.333\textwidth}
      \centering
      \includegraphics[height=3.6cm]{figs/ubt_digitized_hits_gamma.png}\\[2pt]
...
\begin{frame}{Momentum Distribution of Each Particle}
  ...
  \begin{columns}[T]
    \begin{column}{0.333\textwidth}
      \centering
      \includegraphics[height=3.8cm]{../analysis/plots/energy_scan/ubt_hit_momentum_muon.png}\\[1pt]
      \tiny \muminus: up to a few hundred GeV$/c$
    \end{column}
    \begin{column}{0.333\textwidth}
      \centering
      \includegraphics[height=3.8cm]{../analysis/plots/energy_scan/ubt_hit_momentum_electron.png}\\[1pt]
      \tiny \eminus: $\mathcal{O}(1)$\,GeV$/c$
    \end{column}
    \begin{column}{0.333\textwidth}
      \centering
      \includegraphics[height=3.8cm]{../analysis/plots/energy_scan/ubt_hit_momentum_gamma.png}\\[1pt]
      \tiny \gam: $\mathcal{O}(10)$\,MeV$/c$
    \end{column}
  \end{columns}
  \vspace{3pt}
  \footnotesize
  Momentum of particles leaving an \texttt{UpstreamTaggerPoint}.
\end{frame}
...
\begin{frame}{Conclusions}
  \footnotesize
  \begin{itemize}
    \item The standalone Geant4 model \alert{reproduces the
      measured scintillation timing shape} (shared emission spectrum,
      common-rise/fast/slow components) and gives consistent simulated
      PMT-charge distributions for both the $40\times40$ and
      $20\times20~\mathrm{mm^2}$ tiles.
    \item The ADC response is \alert{spatially uniform} once
      energy-normalized, and per-particle ADC-vs-energy calibration curves
      are established for muons, electrons/positrons, and interacted
      photons.
    \item The FairShip digitizer correctly \alert{requantizes} MC points
      from the coarse $40\times40~\mathrm{cm^2}$ transport regions onto the
      physical $2\times2$/$4\times4~\mathrm{cm^2}$ tile grid, applying each
      tile's local ADC/MeV response.
    \item The digitization model gives a \alert{more reliable per-tile rate
      estimate} based on each hit's position in the UBT plane.
  \end{itemize}
\end{frame}
...
\begin{frame}{Next Steps}
  \footnotesize
  \begin{itemize}
    \item \alert{Re-validate the expected per-tile rate map}, evaluate the number of tiles with rate about 1 MHz threshold.
    \item \alert{Address the timing model}: the standalone simulated
      timing does not yet match real tile measurements; FairShip currently
      substitutes a real-data-based timing smearing in its place.
    \item Extend the ADC calibration and rate study to
      additional energies and \alert{angles}, and cross-check against
      real UBT tile test-beam data once available.
    \item Using the ADC-vs-momentum and momentum-distribution results,
      study a possible \alert{rate reduction from an ADC threshold cut}.
  \end{itemize}
\end{frame}
