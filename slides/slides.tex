...
\setlist[itemize]{noitemsep, topsep=1pt, parsep=0pt, itemsep=2pt, leftmargin=1.1em}
...
\begin{frame}{Expected Particle Rate per Tile}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \footnotesize Per-tile expected hit-rate distribution, all species (weighted)
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[height=2.9cm]{figs/ubt_digitized_hits_tiles_above_1mhz.png}\\[2pt]
      \footnotesize Spatial map of tiles exceeding 1\,MHz
    \end{column}
  \end{columns}
  \vspace{4pt}
  \footnotesize
  Digitized \texttt{UpstreamTaggerHit} rates from the MinBias2026
  muon-background sample, weighted to
  ...
  approximate rate in Hz; a few tiles near the beam axis exceed 1\,MHz.
\end{frame}
