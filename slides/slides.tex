...
\begin{frame}{Expected Particle Rate per Tile}
  ...
  $N_\mathrm{total}=4.12\times10^{13}$ muon-background events per spill.
\end{frame}
...
\begin{frame}{Conclusions}
  \footnotesize
  \begin{itemize}
    \item The standalone Geant4 optical/timing model \alert{reproduces the
    ...
    \item Applied to the MinBias2026 muon-background sample, the digitizer
      gives a full \alert{per-tile expected hit-rate map}, split by
      species: muons dominate everywhere, while electrons/positrons and
      photons concentrate near the beam axis, consistent with their softer
      momentum spectra.
  \end{itemize}
\end{frame}
...
\begin{frame}{Next Steps}
  \footnotesize
  \begin{itemize}
    \item \alert{Re-validate the expected per-tile rate map} against the
      most recent tile-ID digitization fixes, and reconcile it with the
      previously reported rates before presenting a final number of tiles
      above the 1\,MHz design threshold.
    \item \alert{Close the timing-model gap}: the standalone simulated
    ...
    \item Extend the per-particle ADC calibration and rate study to
      additional gun energies/species as needed, and cross-check against
      real UBT tile test-beam data once available.
    \item Feed the validated per-tile occupancy/rate map into full
      background- and trigger-rate studies for SHiP.
  \end{itemize}
\end{frame}
